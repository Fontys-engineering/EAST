...
class gdx:
    ...
    def open_ble(self, device_to_open=None):
        ...
        if number_found_devices >= 1: 
            ...
            if open_success == False:
                pass
                #Change print("Error while trying to open device. ")
                #Change print("Troubleshoot by opening Graphical Analysis to test")
        else:
            print("BothF - openBle")
    ...
    def open_selected_device(self):
        ...
        open_success = False
        i = 0
        #Change print("attempting to open", len(gdx.devices), "device(s)...")
        while i < len(gdx.devices): 
            try:
                open_device_success = gdx.devices[i].open()
                #Change print("open device ",i, " = ", open_device_success, sep="")
                if open_device_success:
                    open_success = True
                    gdx.ble_open = True
                else:
            ...
            if type(sensors) == list:
                ...
                if isinstance(sensors[0], list):
                    ...
                    if len(sensors)!= len(gdx.devices):
                        #Change print("the sensor parameter in select_sensors() does not match number of devices")
                        gdx.devices = []
                        return
                    else:
